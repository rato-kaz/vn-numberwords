...
if __name__ == "__main__":
    ...
    test_numbers = [0, 1, 10, 11, 15, 20, 21, 25, 100, 101, 110, 111, 1000, 1001, 1234, 10000, 12345, 100000, 1234567]
    ...
    for number_str in currency_strings:
        try:
            parsed_number = parse_vietnamese_number(number_str)
            result = vietnamese_string_to_currency(number_str, 'đồng', SouthDictionary())
            print(f"{number_str:>20} -> {parsed_number:>15} -> {result}")
        except Exception as e:
            print(f"{number_str:>20} -> Error: {e}")